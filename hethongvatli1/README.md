...
## Docker Quickstart & Deployment

### Run with Docker Compose
```bash
# 1. Clone repo & create environment configuration
cp .env.example .env

# 2. Start PostgreSQL and Spring Boot backend container
...
| Variable | Default (Dev) | Description | Production Example |
|---|---|---|---|
| `DATABASE_URL` | Neon cloud connection | JDBC Connection URL to PostgreSQL | `jdbc:postgresql://db:5432/vatly1` |
| `DATABASE_USERNAME` | `neondb_owner` | Database user | `vatly1_user` |
| `DATABASE_PASSWORD` | - | Database password | `StrongSecretPass123` |
| `JWT_SECRET` | `secret-key` | HMAC SHA256 secret (>= 32 chars) | `vAtLy1_Super_Secure_Long_Secret_Key_2026!` |
| `JWT_EXPIRE_MS` | `300000` (5 mins) | Access token expiration (ms) | `900000` (15 mins) |
| `JWT_REFRESH_EXPIRE_MS` | `604800000` (7 days) | Refresh token expiration (ms) | `604800000` |
| `CORS_ALLOWED_ORIGINS` | `*` | Allowed CORS origins (comma-delimited) | `https://lms.vatly1.edu.vn,http://localhost:3000` |
| `FILE_UPLOAD_DIR` | `uploads` | Storage directory for material files | `/app/uploads` |
| `FILE_MAX_SIZE_MB` | `50` | Maximum file upload size in MB | `50` |
| `SERVER_PORT` | `8080` | Application HTTP listening port | `8080` |

### Production Readiness Checklist
- [x] **Zero Hardcoded Secrets**: `application-prod.yml` uses 100% environment variables.
- [x] **Profile-Aware CORS**: Wildcard `*` disabled when `CORS_ALLOWED_ORIGINS` is configured.
- [x] **Auth Rate Limiting**: `/signin` (10/min/IP) and `/signup` (3/min/IP) throttled with `429 Too Many Requests` (In-memory sliding window for single-node; recommend API Gateway / Redis for multi-pod clusters).
- [x] **Health Check & Actuator**: `/actuator/health` exposed publicly for reverse proxies/load balancers; internal metrics and endpoints strictly restricted to `ROLE_ADMIN` (sensitive endpoints like `/env`, `/heapdump` disabled).
- [x] **Graceful Shutdown**: Enabled with 30s phase timeout to complete in-flight transactions.
- [x] **Multi-stage Dockerfile**: Non-root `appuser` container using Eclipse Temurin 21 JRE Alpine.
- [x] **Dynamic System Settings**: Admin endpoints at `/api/v1/admin/settings` with direct PostgreSQL persistence (zero stale in-memory cache, multi-instance consistency guaranteed).
...
The system includes **173 automated integration test cases** across **20 test suites** with 100% PASS rate:
- `ActivityAndAuditLogTest` (9 tests)
- `ActuatorHealthTest` (2 tests)
- `AiTutorControllerTest` (10 tests)
- `AnalyticsControllerTest` (17 tests)
- `ClassControllerTest` (9 tests)
- `DashboardControllerTest` (5 tests)
- `EvidenceControllerTest` (6 tests)
- `ExamControllerTest` (14 tests)
- `ExperimentControllerTest` (10 tests)
- `IdorSecurityControllerTest` (15 tests)
- `LearningMaterialControllerTest` (6 tests)
- `LearningProgressControllerTest` (11 tests)
- `QuestionBankControllerTest` (8 tests)
- `RateLimitControllerTest` (2 tests)
- `SemesterControllerTest` (7 tests)
- `StudentClassControllerTest` (2 tests)
- `SubjectControllerTest` (6 tests)
- `SystemSettingControllerTest` (5 tests)
- `TopicControllerTest` (2 tests)
